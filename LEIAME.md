...
| `batch-eviction.js` | Stop | acima de `EVICTION_LIMIT` (150k), uma vez por sessão, segura o stop e faz o modelo escrever o handoff em `<raiz do projeto>/.claude/handoff-<sessão>.md` (`CLAUDE_PROJECT_DIR`, não a pasta em que a sessão deu `cd`). |
| `handoff-load.js` | SessionStart (`startup\|clear`) | depois de `/clear` ou sessão nova, percorre os handoffs do mais novo ao mais velho e injeta o primeiro que consegue carregar (< `HANDOFF_HOURS`, 72; `0` desliga o handoff, p.ex. para um lote de `claude -p`; escrito por sessão que rodou nesta máquina; nunca carregado antes). Sessões que partem no mesmo instante se revezam por um arquivo de trava ao lado do `.context-peaks.json` (medido antes dela: um arquivo chegou a 2-6 de 28 partidas paralelas). O arquivo é procurado em `CLAUDE_PROJECT_DIR/.claude`, a raiz onde a sessão começou, porque o `cwd` do stdin segue o `cd` da ferramenta Bash (antes disso, 45 de 213 handoffs numa máquina foram escritos em subpasta e 26 nunca carregaram). Arquivo estrangeiro ou já carregado não esconde um mais velho que seja seu; arquivo que existe mas não foi carregado é nomeado ao modelo, com o motivo. A carga é registrada (pelo mtime do arquivo, no `.context-peaks.json`) antes de o texto entrar: se a marca não grava, nada é injetado e o próximo `/clear` tenta de novo. O texto entra como arquivo `<handoff>` (os primeiros 12000 chars; uma nota depois do bloco dá o caminho do resto) que o modelo deve conferir contra a árvore de trabalho. Depois instala o **canário de contexto**: uma primeira linha byte-estável (`**Lastro · t<N> · ctx ok**`) que some quando a instrução caiu do contexto; o canário nunca depende do handoff. |
